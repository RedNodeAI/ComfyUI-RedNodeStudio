...
| RedNode Sampler Start / Sampler End | Your own sampler chain for a rig. Build any sampler between them, a KSampler, SamplerCustomAdvanced with its own sigmas and guider, another pack's sampler, name it at Sampler End, and pick that name as a rig's Sampler chain on the Models tab. Every render, pass and Detailer pass on that rig samples through it with its own prompts, latent and denoise. Nothing is wired. |
