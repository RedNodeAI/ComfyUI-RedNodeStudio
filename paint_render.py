...
class RedNodePaintRender:
    ...
    def render(self, model=None, positive=None, negative=None, vae=None,
        ...
        # loader Paint Out uses, so the two paint doors can never disagree about
        # which LoRAs a rig arrives with. A WIRED input is used exactly as wired,
        # because it usually already carries the main stack and stacking on top of
        # it is the double-apply this file already warns about.
        if model is None or clip is None or vae is None:
            try:
                from .workspace import load_paint_rig, parse_config as _pcfg
                _full = _pcfg(json.dumps(_workspace_cfg(prompt)))
                _, _rm, _rc, _rv = load_paint_rig(_full)
                if model is None and _rm is not None:
                    model = _rm
